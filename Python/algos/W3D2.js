...
function binarySearch(sortedNums, searchNum) {
    ...
    while (start <= end ){
        ...
        // rounds down for our halfing function to make 'focus block'
        if (sortedNums[mid] == searchNum){
            return true 
        }
        // checking if our search num is less than  sortednums @ mid 
        else if  ( searchNum < sortedNums[mid] ) {
            var end = mid - 1
        }
        // setting end to mid -1 , this moves our block of focus 
        else {
            var start = mid + 1
        }
        // same, but moving start position 
    }
    return false
