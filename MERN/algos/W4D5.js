...
const recipe1 = {
  ...
  function getMaxServings(recipe, available) {
    let possibleServings = Infinity;
    for(let [key, value] of Object.entries(recipe)) {
      if (!available.hasOwnProperty(key)) {
        return 0;
      }
      let tempServing = available[key] / value
      if (tempServing < possibleServings) {
        possibleServings = tempServing;
      }
    }
  ...
  //             return 0;
  //         }
  //         let x = available[ingredient] / recipe[ingredient];
  //         if (!servings || x < servings) {
  //             servings = x;
  //         }
  //     }
  //     return servings;
